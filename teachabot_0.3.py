            ...
            if len(lines) < 3:
                ...
                #return something that can make a if n send mesage
                    ...
                    if 'your' in lines[-2] or 'Your' in lines[-2]:
                        ...
                        message = message.replace('am ', 'are ', 10) #cause i AM ...
                        message = message.replace('Am ', 'Are ', 10)

                        message = message.replace('I ', 'You ', 10) #cause I am...
...
##        self.history.update_chat_history('[color=20dd20]{}[/color] > {}'.format('Bot: ', message))
##        
...
class Brain:
    ...
    def retrieve_entry(self, question):
        words = question.strip().split(' ')#create list of words from question
        shortlisted = []
        for key in self.brain:
            if key == question:
                ...
                count = 0
                for word in words:                      #these are common words
                    if word in key and word.lower() not in ['what', 'who', 'where', 'when', 'why', 'how', 'you', 'your', 'are', 'i', 'am', 'my', 'we', 'they']:
                        count += 1
                #if theres even 3 word hit, a logical sentence would probably need at least 3, 1 is abit little
                if count > 2:
                    #check the length difference between count and the word,
                    #ifthe difference is too much, its unrealiable to shortlisrtt it
                    difference = abs(len(words) - count)
                    if difference <= 3: #if a sentence has 10 words, but only just nicely 3 words hit, it shouldnt have the same answer most probably
                        shortlisted.append([key, count])
...
class TeachABot(App):
    def readme(self,*args):
        ...
        box.add_widget(Label(text="Thanks for trying out Teach A Bot!\n A few commands you should know:\n /skip : Skips answering a Bot's question\n /wipe : Wipe the memory of the Bot completely\n\nREMEMBER: It learns from entirely YOU, be nice to it so it will be nice to you ;) \nHope you enjoy!",
                             halign="center", valign = "middle"))

        close_btn = Button(text = "Close", size_hint = (0.2, 0.2), pos_hint = {'centre_x':1, "bottom":1})
        
        box.add_widget(close_btn)

        popup = Popup(title="ReadMe", content = box,
                      size_hint=(0.7, 0.6),
                      auto_dismiss=True)
